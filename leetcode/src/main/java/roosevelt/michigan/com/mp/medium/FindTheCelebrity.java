...
import roosevelt.michigan.com.common.Relation;
...
public class FindTheCelebrity extends Relation {
	...
	public int findCelebrity(int n) {
		...
		Set<Integer> celebs = new HashSet<>();
		for (int i = 0; i < n; i++) {
			celebs.add(i);
		}
	...
	private void prunCelebs(Set<Integer> celebs, int a) {
		Set<Integer> prunedCelebs = new HashSet<Integer>();

		for (Integer i : celebs) {
			if (knows(a, i)) {
				prunedCelebs.add(a);
			} else {
				prunedCelebs.add(i);
